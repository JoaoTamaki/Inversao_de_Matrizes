...
Foi possível explorar bem a parte de erros no que foi implementado, foi feito testes de mesa para acompanha a execução dos algoritmos até o que foi possível, foi possível progredir bastante mesmo devido os problemas, foi modularizado com funções para cada etapa foi tratado entrada e saída corretamente, foi feito um make funcional, foi respeitado ao enunciado, mesmo que não esteja completo e está sendo entregue fora do prazo, e por pouco que não foi terminado tudo, que foram limpos para uma entrega do que foi possível ser feito.